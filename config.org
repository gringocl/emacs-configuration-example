...
(defun c/insert-log-date (&optional days-back)
  "Insert date(s) in my typical log format (e.g. Fri Jan 7 2022).
With prefix argument DAYS-BACK, insert date that many days in the past.
With C-u (no number), prompt for number of days back.
With double prefix C-u C-u, prompt for days and generate a sequence
of past dates, ending with today."
  (interactive "P")
  (let* ((do-sequence (equal days-back '(16))) ; C-u C-u
         (days-to-past
          (cond
           ((null days-back) 0)
           ((eq days-back '(4))  ; C-u
            (string-to-number
             (read-string "Days back: " "1")))
           (do-sequence
            (string-to-number
             (read-string "Generate sequence for how many past days: " "3")))
           (t (prefix-numeric-value days-back))))
         (time-spec (if (not do-sequence)
                       ;; Single date case
                       (time-subtract (current-time)
                                    (days-to-time days-to-past))
                     ;; Sequence case
                     (let ((dates '()))
                       (dotimes (i days-to-past)
                         (push (format-time-string "%a %b %-e %Y"
                                                 (time-subtract (current-time)
                                                              (days-to-time i)))
                               dates))
                        dates))))
    (if (not do-sequence)
        (insert (format-time-string "%a %b %-e %Y" time-spec))
      (insert (string-join time-spec ", ")))))
...
If FORCE-P is non-nil, overwrite destination file without confirmation."
  ...
  (let ((old-path (buffer-file-name (buffer-base-buffer)))
    ...
    (when (featurep 'magit)
      (if-let* ((default-directory (magit-toplevel (file-name-directory new-path))))
          (magit-refresh)))
    (find-file new-path)
...
#+begin_src emacs-lisp
(defun c/consult-ripgrep-in-directory ()
    "Find a directory and then search in that directory."
    (interactive)
    (consult-ripgrep t))
#+end_src
...
#+begin_src emacs-lisp
(defun c/slugify-title (title)
  "Slugify TITLE into kebab-case suitable for a filename and copy to the kill ring.
Lowercases, replaces runs of non-alphanumeric characters with a single dash,
and trims leading/trailing dashes. The current kill is offered as the default."
  (interactive
   (list (read-string "Title: " (and kill-ring (current-kill 0 t)))))
  (let* ((downcased (downcase title))
         (dashed (replace-regexp-in-string "[^[:alnum:]]+" "-" downcased))
         (slug (replace-regexp-in-string "\\(^-+\\|-+$\\)" "" dashed)))
    (when (string-empty-p slug)
      (user-error "Title produced an empty slug"))
    (kill-new slug)
    (message "Copied slug: %s" slug)
    slug))
#+end_src
...
(use-package leader-key
  ...
  (leader-key-declare-prefix "S" "spell" "Spell")
...
#+begin_src emacs-lisp
(defconst c/env-file (no-littering-expand-var-file-name "env"))

(defvar c/env-ignored-vars
  '("^DBUS_SESSION_BUS_ADDRESS$"
    ...
    "^TERM$"
    "^ITERM")
  "Environment variables to not save in `c/env-file'.
Each string is a regexp, matched against variable names to omit from
`c/env-file'.")

(defun c/rpartial (fn &rest args)
...
(defun c/reload-env-file (&optional force-p env-file)
  "Generates `c/env-file', if it doesn't exist (or if FORCE-P).
This scrapes the variables from your shell environment by running
`c/env-executable' through `shell-file-name' with `c/env-switches'. By
default, on Linux, this is '$SHELL -ic /usr/bin/env'. Variables in
`c/env-ignored-vars' are removed."
      ...
      (with-temp-file env-file
        (message "%s envvars file at %S"
                 (if (file-exists-p env-file)
                   ...
                   "Generating")
                 env-file)
        (message "Scraping shell environment")
        ...
        (insert
         (concat
          ...
          "# ---------------------------------------------------------------------------\n"
          "# This file was auto-generated by `make env'. It contains a list of environment\n"
          "# variables scraped from your default shell (excluding variables blacklisted\n"
          "# in doom-env-ignored-vars).\n"
          "#\n"
          (if (file-equal-p env-file c/env-file)
        ...
        (dolist (env process-environment)
          (if (cl-find-if (c/rpartial #'string-match-p (car (split-string env "=")))
                          c/env-ignored-vars)
              (message "Ignoring %s" env)
            (insert env "\n")))
        (message "Successfully generated %S" env-file)
        t))))

(defun c/load-envvars-file (file &optional noerror)
...
(when (and (or (display-graphic-p)
               (daemonp))
           (file-exists-p c/env-file))
  (c/load-envvars-file c/env-file))
#+end_src
...
(use-package evil-multiedit
  :commands (evil-multiedit-match-and-next)
  :config
  (evil-multiedit-default-keybinds))

(use-package iedit
  :ensure (iedit :host github :repo "victorhge/iedit" :protocol ssh)
  :init
...
(use-package ultra-scroll
  :ensure (ultra-scroll :host github :repo "jdtsmith/ultra-scroll" :protocol ssh)
  :config
  (ultra-scroll-mode 1))
#+end_src
...
(use-package ripgrep-capf
  ...
  (defalias 'capf-dabbrev-ripgrep (cape-capf-super #'ripgrep-capf #'cape-dabbrev)))
#+end_src

*** DISABLED Ripgrep Line Capf

#+begin_src emacs-lisp
(use-package rg-line-capf
  :ensure (rg-line-capf :repo "~/gringocl/rg-line-capf")
  :config
  ;; Configure search directories - point to your repos
  (setq rg-line-search-directories '("~/wsgr/neuron"))

  ;; Add to completion-at-point-functions
  (add-to-list 'completion-at-point-functions #'rg-line-capf))
#+end_src
...
(use-package consult
  ...
  (setq consult-ripgrep-args
        "rg --null --line-buffered --color=never --max-columns=100000 --path-separator / --smart-case --no-heading --with-filename --line-number --search-zip"))
#+end_src
...
Jinx provides camelCase-aware spell checking via libenchant. Identifiers
like ~postalCode~ are split into ~postal~ + ~Code~ and each fragment is
checked separately, so the whole token only flags when a real fragment is
misspelled. ~jinx-languages~ holds both English and Spanish dictionaries;
a word passes if either accepts it.

Prereq: ~brew install enchant~ (uses the existing aspell dictionaries as
backend).

#+begin_src emacs-lisp
(use-package jinx
  :hook ((org-mode git-commit-mode markdown-mode prog-mode) . jinx-mode)
  :bind (([remap ispell-word] . jinx-correct)
         ("C-M-$" . jinx-languages))
  :config
  (setq jinx-languages "en_US es"))
#+end_src
...
(use-package flycheck
  :hook (c/first-buffer . global-flycheck-mode)
  :ensure (flycheck :host github :repo "aaronjensen/flycheck" :branch: "ruby-3.4"  :protocol ssh))
#+end_src
...
** All the icons
When on new installation remember to rum `M-x all-the-icons-install-fonts`
...
(use-package magit
  ...
  (define-key magit-file-section-map [remap magit-diff-visit-file] #'magit-diff-visit-worktree-file)
  (define-key magit-hunk-section-map [remap magit-diff-visit-file] #'magit-diff-visit-worktree-file)

  ;; Refined (word-level) diffs everywhere, not just the hunk under point.
  ;; The whole-line backgrounds are left to the active theme; we only override
  ;; the within-line refinement overlays so the actually-changed characters
  ;; stand out.
  (setq magit-diff-refine-hunk 'all)

  (custom-set-faces
   '(diff-refine-added   ((t (:background "#7fcf7f" :foreground "#0a2a0a" :weight bold))))
   '(diff-refine-removed ((t (:background "#f08a8a" :foreground "#330808" :weight bold)))))

  ;; (add-to-list 'savehist-additional-variables 'git-commit-previous-message)
...
(use-package browse-at-remote
  ...
  :config
  (defun c/browse-at-remote--github-plain-for-markdown (orig repo-url location filename &optional linestart lineend)
    "Append ?plain=1 to GitHub markdown URLs when a line is targeted, so the source view is shown instead of the rendered preview."
    (let ((url (funcall orig repo-url location filename linestart lineend)))
      (if (and linestart
               (string-match-p "\\.\\(md\\|markdown\\)\\'" filename)
               (string-match "\\`\\(.*?\\)\\(#L[0-9]+\\(?:-L[0-9]+\\)?\\)\\'" url))
          (concat (match-string 1 url) "?plain=1" (match-string 2 url))
        url)))
  (advice-add 'browse-at-remote--format-region-url-as-github
              :around #'c/browse-at-remote--github-plain-for-markdown)
...
(use-feature org
  ...
  (setq org-capture-templates
        '(("c" "Config To Do" entry (file+headline "~/.emacs.d/notes.org" "To Dos")
           "* TODO %?\n  %i\n  %a")
          ("C" "Config To Do + ask Claude" entry (file+headline "~/.emacs.d/notes.org" "To Dos")
           "* TODO %?\n  %i\n  %a"
           :c/ask-claude t)))
  ...
  (defun c/capture-config-todo-and-ask ()
    "Capture a config TODO and dispatch a background Claude session to research it."
    (interactive)
    (org-capture nil "C"))

  (leader-key-set "c" #'c/capture-config-todo)
  (leader-key-set "C" #'c/capture-config-todo-and-ask))
#+end_src

*** Claude Capture Dispatch

~SPC C~ captures a config TODO and dispatches a background ~claude -p~
session against the repo root. The session reads the note, analyzes the
config, and writes its findings back under the captured headline as a
~:CLAUDE:~ drawer. Plain ~SPC c~ still works for capture-only.

Failures land in ~*claude-capture-log*~. The prompt asks for analysis +
proposal (diff if small, bullets if needs discussion, "needs a real
conversation" if open-ended). Multiple captures run concurrently.

#+begin_src emacs-lisp
(defvar c/claude-capture-prompt
  "You are being invoked from an Emacs org-capture hook. The user just
jotted down a note about something they want changed or investigated in
their personal Emacs config at this repo's root.

The note text is below, between BEGIN-NOTE and END-NOTE.

Your job:
1. Read the relevant parts of the config (config.org is the source of
   truth; init.el and early-init.el are also hand-written).
2. Decide whether the note describes (a) a small, self-contained change
   you can propose as a unified diff, (b) a research/analysis question
   that needs a written answer, or (c) something too large or too
   subjective for a one-shot answer.
3. Respond in this exact format, suitable for pasting into an org file:

   *** Analysis
   <one to three short paragraphs of your findings>

   *** Proposal
   <one of:>
   - A unified diff in a ~#+begin_src diff~ block IF the change is small
     and you are confident.
   - A short bulleted list of next steps IF the change needs discussion.
   - The phrase \"Needs a real conversation.\" IF the question is open-ended.

Do NOT edit any files. Do NOT run commands beyond reading. Keep total
output under 400 words. The user will read this when they're ready and
apply it themselves.

BEGIN-NOTE
%s
END-NOTE"
  "Prompt template passed to `claude -p' for capture-and-ask notes.
%s is replaced with the captured note text.")

(defun c/claude-capture--dispatch (note-text marker)
  "Run `claude -p' against NOTE-TEXT and insert the result at MARKER on completion."
  (let* ((prompt (format c/claude-capture-prompt note-text))
         (log-buf (get-buffer-create "*claude-capture-log*"))
         (output-buf (generate-new-buffer " *claude-capture-output*"))
         (default-directory (expand-file-name "~/.emacs.d/"))
         (proc (make-process
                :name "claude-capture"
                :buffer output-buf
                :command (list (or (executable-find "claude") "claude")
                               "-p" prompt)
                :stderr log-buf
                :noquery t
                :sentinel
                (lambda (process event)
                  (when (memq (process-status process) '(exit signal))
                    (let ((output (with-current-buffer (process-buffer process)
                      ...
                      (if (zerop exit-code)
                          (c/claude-capture--insert-result marker output)
                        (with-current-buffer log-buf
                          (goto-char (point-max))
                          (insert (format "\n[%s] claude exited %d for note: %s\n"
                                          (format-time-string "%FT%T")
                                          exit-code
                                          (truncate-string-to-width note-text 80))))
                        (message "Claude capture failed (exit %d). See *claude-capture-log*."
                                 exit-code))
                      (set-marker marker nil)))))))
    (message "Claude is thinking about: %s"
             (truncate-string-to-width note-text 60 nil nil "..."))
    proc))

(defun c/claude-capture--insert-result (marker output)
  "Insert OUTPUT under the captured headline at MARKER."
  (when (and (markerp marker) (marker-buffer marker))
    (with-current-buffer (marker-buffer marker)
      (save-excursion
        ...
        (insert "\n:END:\n")
        (save-buffer))
      (message "Claude finished. Result inserted into notes.org."))))

(defun c/claude-capture-after-finalize ()
  "Org-capture hook: if the just-finalized capture had :c/ask-claude, dispatch Claude."
  (when (and (not org-note-abort)
             (plist-get org-capture-plist :c/ask-claude))
    (let* ((entry-marker (org-capture-get :insertion-point))
           (note-text
            (when (and entry-marker (marker-buffer entry-marker))
      ...
      (when (and note-text (> (length (string-trim note-text)) 0))
        (c/claude-capture--dispatch
         note-text
         (copy-marker entry-marker))))))

(add-hook 'org-capture-after-finalize-hook #'c/claude-capture-after-finalize)
#+end_src

*** Org Superstar / Appear
...
;;   :after (dired)
;;   :commands (diredp-do-hardlink-recursive)
;;   :ensure (dired+ :host github :repo "emacsmirror/dired-plus" :protocol ssh))
...
(use-package dirvish
  ...
  :init
  (setq dirvish-cache-dir (concat no-littering-var-directory "dirvish"))
  ...
  (leader-key-set "o p" #'dirvish-side)
...
(use-package haml-mode
  :preface
  (defun c/haml-correct-comment-variables ()
    "Correct an issue with uncomment when using evil-nerd-commenter"
    (setq-local comment-start "-# ")
    (setq-local comment-start-skip "-#+ *"))

  :hook (haml-mode. c/haml-correct-comment-variables)
...
A major mode for editing template-markup and docx-markup files used in the
Neuron ~template-markup~ and ~docx-markup~ libraries. Both languages share the
same ={tag:}= / ={:tag}= envelope, so a single mode handles both.

The mode highlights:

- Template-markup control-flow tags (~if~, ~each~, ~delimited-list~,
  ~enumerated-list~, ~render~, ~insert~, …) and their close tags.
- Docx-markup formatting tags — character styles (~bold~, ~italic~, …),
  paragraph/section styles, tables, breaks, hyperlinks, bookmarks, and the
  RSPA / indemnification-agreement section tags.
- Delimiter schemes (~comma-and~, ~semicolon-or~, …) and enumerator schemes
  (~letter~, ~roman~, ~number~, …).
- Named parameters (~as=~, ~url=~, ~columns=~, …), variable references with
  optional dotted access and predicate ~?~ suffixes, and quoted strings.
- Escape sequences per the grammar (~\{~, ~\}~, ~\\~, ~\<any-char>~).
- Invisible characters that frequently sneak in via copy/paste from Word —
  literal U+00A0 NON-BREAKING SPACE and U+200B ZERO-WIDTH SPACE — are shown
  with a high-contrast face so they are visually distinct from regular spaces.

** Faces

#+begin_src emacs-lisp
(defface template-markup-invisible-character-face
  '((((background dark))  :background "#552222" :foreground "#ffaaaa" :weight bold)
    (t                    :background "#ffd0d0" :foreground "#aa0000" :weight bold))
  "Face for highlighting invisible characters (NBSP, ZWSP) in template/docx markup."
  :group 'template-markup)

(defface template-markup-escape-face
  '((t :inherit font-lock-warning-face))
  "Face for escape sequences (e.g. \\{ \\} \\\\) in template/docx markup."
  :group 'template-markup)

(defface template-markup-tag-bracket-face
  '((t :inherit font-lock-bracket-face))
  "Face for the {, }, and {: tag delimiters in template/docx markup."
  :group 'template-markup)
#+end_src
...
#+begin_src emacs-lisp
(defvar template-markup-control-flow-keywords
  '("if" "if-not" "else" "else-if" "else-if-not"
    "each" "insert" "render"
    "delimited-list" "delimiter"
    "enumerated-list" "enumerator"
    "no-leading-space")
  "Template-markup control-flow tag names (the original `template-markup' gem).")

(defvar template-markup-docx-tags
  '(;; Character styles
    "bold" "italic" "underline" "uppercase" "error"
    "signature-field" "inline-signature-field" "signing-role" "party-name"
    ;; Hyperlink and bookmark
    "hyperlink" "bookmark" "bookmark-link" "bookmark-link-full" "bookmark-link-relative"
    ;; Spacing and special characters
    "tab" "line-break" "column-break" "page-break" "section-break"
    "white-space" "non-breaking-space" "non-breaking-hyphen"
    ;; Tables
    "table" "table-row" "table-row-header" "table-cell"
    ;; Paragraphs and section styles
    "p" "heading-1" "center" "signature" "date-signature"
    "signature-block" "signature-block-signature"
    "signature-page-preamble" "signature-page-footer"
    "prescribed-legend" "footer"
    ;; RSPA section tags
    ...
    "rspa-exhibit" "rspa-exhibit-indent" "rspa-exhibit-title"
    "rspa-page-number" "rspa-section-title-page"
    ;; Indemnification-agreement section tags
    "indemnification-agreement-section-level-1"
    "indemnification-agreement-section-level-2"
    "indemnification-agreement-section-level-3"
    "indemnification-agreement-section-level-4"
    "indemnification-agreement-recital"
    "indemnification-agreement-page-number"
    "indemnification-agreement-section-title-page"
    ;; Generic exhibit
    "exhibit-title")
  "Docx-markup tag names.")

(defvar template-markup-all-tag-names
  (append template-markup-control-flow-keywords
          template-markup-docx-tags)
  "All known tag names across template-markup and docx-markup.")

(defvar template-markup-delimiter-schemes
  '("comma" "comma-and" "comma-or"
    "semicolon" "semicolon-and" "semicolon-or")
  "Delimiter schemes for {delimited-list:} / {enumerated-list: delimiter-scheme=}.")

(defvar template-markup-enumerator-schemes
  '("letter" "letter-uppercase" "letter-x" "number" "roman")
  "Enumerator schemes for {enumerated-list:}.")
#+end_src

** Syntax table and font-lock keywords

#+begin_src emacs-lisp
(defvar template-markup-mode-syntax-table
  (let ((table (make-syntax-table)))
    (modify-syntax-entry ?_  "w"  table) ; underscore is a word constituent
    (modify-syntax-entry ?-  "w"  table) ; hyphen is a word constituent (tags use it)
    (modify-syntax-entry ?.  "."  table)
    (modify-syntax-entry ?\" "\"" table)
    (modify-syntax-entry ?\\ "\\" table)
    table)
  "Syntax table for `template-markup-mode'.")

(defvar template-markup-font-lock-keywords
  (let* ((all-tags-rx           (regexp-opt template-markup-all-tag-names t))
         (control-flow-rx       (regexp-opt template-markup-control-flow-keywords t))
         (delimiter-schemes-rx  (regexp-opt template-markup-delimiter-schemes t))
         (enumerator-schemes-rx (regexp-opt template-markup-enumerator-schemes t)))
    `(;; Escape sequences: \{  \}  \\  \<any-char>  -- match these first so braces
      ;; inside escapes don't get parsed as tag delimiters.
      ("\\\\." 0 'template-markup-escape-face t)

      ;; Invisible characters: NBSP (U+00A0), ZWSP (U+200B). High priority so they
      ;; are visible no matter where they appear.
      ("[ ​]" 0 'template-markup-invisible-character-face t)

      ;; Close tag: {:tagname}
      (,(concat "\\({:\\)" all-tags-rx "\\(}\\)")
       (1 'template-markup-tag-bracket-face)
       (2 font-lock-keyword-face)
       (3 'template-markup-tag-bracket-face))

      ;; Close tag with unknown tag name: {:something}  -- still highlight the brackets
      ("\\({:\\)\\([a-z][a-z0-9-]*\\)\\(}\\)"
       (1 'template-markup-tag-bracket-face)
       (2 font-lock-warning-face)
       (3 'template-markup-tag-bracket-face))

      ;; Opening tag with parameters: {tagname:  or  {tagname<space>...}
      (,(concat "\\({\\)" all-tags-rx "\\(?::\\|[ }]\\)")
       (1 'template-markup-tag-bracket-face)
       (2 font-lock-keyword-face))

      ;; Opening tag with unknown tag name: {something:  or  {something<space>...}
      ("\\({\\)\\([a-z][a-z0-9-]*\\)\\(?::\\|[ }]\\)"
       (1 'template-markup-tag-bracket-face)
       (2 font-lock-warning-face))

      ;; Delimiter scheme symbol immediately after `delimited-list:' or
      ;; `delimiter-scheme=' -- highlight as a type/enum.
      (,(concat "\\(?:delimited-list:\\|delimiter-scheme=\\)\\s-*"
                delimiter-schemes-rx "\\b")
       (1 font-lock-type-face))

      ;; Enumerator scheme symbol immediately after `enumerated-list:'
      (,(concat "enumerated-list:\\s-*" enumerator-schemes-rx "\\b")
       (1 font-lock-type-face))

      ;; Named parameters: key=value (key only)
      ("\\b\\([a-z][a-z0-9-]*\\)=" (1 font-lock-variable-name-face))

      ;; Variable references inside control-flow tags: {if: some.var?}, {each: xs as=...}
      (,(concat "{" control-flow-rx ":\\s-*\\([a-z][a-z0-9_]*\\(?:\\.[a-z][a-z0-9_]*\\)*\\??\\)")
       (2 font-lock-variable-name-face))

      ;; Predicate variables (ending in ?)
      ("\\b\\([a-z_][a-z0-9_]*\\?\\)" (1 font-lock-constant-face))

      ;; Quoted strings
      ("\"[^\"]*\"" . font-lock-string-face)

      ;; Numbers in tag parameters
      ("\\b\\(-?[0-9]+\\(?:\\.[0-9]+\\)?\\)\\b" (1 font-lock-constant-face))

      ;; Stray braces (shouldn't usually match, but ensures any leftover { or } gets
      ;; the bracket face).
      ("[{}]" 0 'template-markup-tag-bracket-face)))
  "Font lock keywords for `template-markup-mode'.")
#+end_src

** Indentation

#+begin_src emacs-lisp
(defun template-markup-indent-line ()
  "Indent current line for template markup."
  (interactive)
  (let ((indent 0)
        (pos (- (point-max) (point))))
    (save-excursion
      ...
      (when (not (bobp))
        ...
        (when (and (looking-at ".*{\\(if\\|if-not\\|each\\|delimited-list\\|enumerated-list\\):")
                   (not (looking-at ".*{:[a-z-]+}")))
...
** Mode definition and auto-mode

#+begin_src emacs-lisp
(define-derived-mode template-markup-mode prog-mode "Template-Markup"
  "Major mode for editing Neuron template-markup and docx-markup files."
  :syntax-table template-markup-mode-syntax-table
  (setq-local comment-start nil)
  (setq-local comment-end nil)
  (setq-local font-lock-defaults '(template-markup-font-lock-keywords nil nil))
  (setq-local indent-line-function #'template-markup-indent-line)
  ;; Long paragraphs of prose are normal here -- wrap at the window edge.
  (visual-line-mode 1))

(add-to-list 'auto-mode-alist '("\\.tmpl\\'"     . template-markup-mode))
(add-to-list 'auto-mode-alist '("\\.template\\'" . template-markup-mode))
#+end_src

** Content-based detection for .txt files

The fixtures and proof templates in ~template-markup~ and ~docx-markup~ live in
plain ~.txt~ files. Sniff the first ~4 KiB for a markup tag before falling
through to ~text-mode~.

#+begin_src emacs-lisp
(defun template-markup-buffer-looks-like-markup-p ()
  "Return non-nil if the current buffer's prefix contains a template/docx-markup tag.
Used as the predicate side of a `magic-mode-alist' entry, so the test runs
against the buffer that's about to be visited."
  (let* ((tag-name-rx (regexp-opt template-markup-all-tag-names t))
         (probe (concat "{" tag-name-rx "[}: ]")))
    (save-excursion
      (save-restriction
        (widen)
        (goto-char (point-min))
        (re-search-forward probe (min (point-max) 4096) t)))))

(add-to-list 'magic-mode-alist
             '(template-markup-buffer-looks-like-markup-p . template-markup-mode))
#+end_src
